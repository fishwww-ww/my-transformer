...
class MyTransformer(Transformer):
    def __init__(self, input_dim, d_model=64, num_heads=4, num_layers=2, d_ff=256, max_seq_length=100, dropout=0.1, use_softmax=False):
        ...
        self.use_softmax = use_softmax  # 控制是否使用softmax输出
        ...
        # 如果需要softmax，添加额外的线性层和softmax
        if use_softmax:
            self.softmax_layer = nn.Sequential(
                nn.Linear(d_model, 2),  # 假设我们想要二分类概率
                nn.Softmax(dim=-1)
            )
    ...
    def forward(self, src):
        ...
        if self.use_softmax:
            # 使用softmax输出概率分布
            output = self.softmax_layer(final_step_feature)  # [batch_size, 2]
        else:
            # 原始回归输出
